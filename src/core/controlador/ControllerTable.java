...
import core.services.AgeService;

public class ControllerTable {

    public void showFlights(JTable table) {
        try {
            CalculateArrivalDate calculate = new CalculateArrivalDate();
            ...
            for (Flight flight : flights) {
                create.addRow(new Object[]{flight.getId(), flight.getDepartureLocation().getAirportId(), flight.getArrivalLocation().getAirportId(), (flight.getScaleLocation() == null ? "-" : flight.getScaleLocation().getAirportId()), flight.getDepartureDate(), calculate.calculate(flight), flight.getPlane().getId(), flight.getNumPassengers()});
            }
    ...
    public void showPassengers(JTable table) {
        try {
            ...
            FullFormat fullFormat = new FullFormat();
            AgeService age =  new AgeCalculator();
            for (Passenger passenger : passengers) {
                create.addRow(new Object[]{passenger.getId(), fullFormat.getFullname(passenger), passenger.getBirthDate(), age.calculateAge(passenger), passenger.getPhone(), passenger.getCountry(), passenger.getNumFlights()});
            }
            table.setModel(create);
        } catch (Exception e) {
            System.out.println("Error, contacte al administrador");
    ...
    public static Response showMyFlights(JTable table, String passengerId) {
        ...
        try {
            ...
            if (passenger == null) {
             ...
             CalculateArrivalDate calculateArrival = new CalculateArrivalDate();
            for (Flight flight : passenger.getFlights()) {
                model.addRow(new Object[]{
                    flight.getId(),
                    flight.getDepartureDate(),
                    calculateArrival.calculate(flight)
                });
            }
